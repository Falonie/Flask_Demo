from flask import url_for, redirect, request, render_template, session, jsonify, current_app, views
from flask_login import login_required, current_user
from flask_paginate import Pagination, get_page_parameter
from . import main
from .. import db, mail
from ..models import User, Question, Comment
from .forms import PostForm, CommentForm, EditProfileForm, EditArticleForm
...
def index():
    ...
    pagination = Question.query.paginate(page=page, per_page=current_app.config['FLASKY_POSTS_PER_PAGE'],
                                         error_out=False)
...
def articles(question_id):
    ...
    page = request.args.get('page', type=int, default=1)
    pagination = Comment.query.filter(Comment.question_id == question_id).paginate(page=page, per_page=
...
def delete_comment():
    ...
    print('$' * 50)
    print('comment_id {}'.format(comment_id))
    question_id = request.form.get('question_id')
    print('$' * 50)
...
def edit_profile():
    ...
    print('username is {}'.format(current_user.username))
...
def posts(author):
    ...
    # pagination = Question.query.filter(User.id == current_user.id).paginate(page=page, per_page=current_app.config
    user = User.query.filter(User.username == author).first()
    print('user is {}'.format(user))
    pagination = Question.query.filter(User.id == user.id).paginate(page=page, per_page=current_app.config
    ['FLASKY_ARTICLES_PER_PAGE'], error_out=False)
    print('pagination is {}'.format(pagination))
    posts = pagination.items
    post1 = Question.query.filter(User.id == user.id).first()
